...
public:
	...
	Eigen::MatrixXd backwardPass(const Eigen::MatrixXd& upstream) override {
		Eigen::MatrixXd deltaThroughReLU = upstream.cwiseProduct(interimOutput.unaryExpr<double(*)(double)>(&Helper::reluPrime));
	...
	Eigen::MatrixXd forwardPass(const Eigen::MatrixXd& input) override {
		...
		output = interimOutput.unaryExpr<double(*)(double)>(&Helper::relu);
		return output;
	}
